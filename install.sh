...
# СКАЧИВАНИЕ С ТВОЕГО РЕПОЗИТОРИЯ
...
# --- ШАГ 2: АДМИНЫ ---
echo -e "\n${YELLOW}2️⃣  ШАГ ВТОРОЙ: Ваш ID (Админ)${NC}"
echo "   1. Найдите бота @userinfobot"
echo "   2. Нажмите Start"
echo "   3. Скопируйте цифры из строки Id"
echo -n "👉 Введите ваш ID (только цифры): "
read ADMIN_ID

# --- ШАГ 3: ГРУППА ---
echo -e "\n${YELLOW}3️⃣  ШАГ ТРЕТИЙ: ID группы поддержки${NC}"
echo "   1. Создайте новую группу в Telegram"
echo "   2. Добавьте туда вашего созданного бота (из шага 1)"
echo "   3. Сделайте бота АДМИНИСТРАТОРОМ группы"
echo "   4. Добавьте в группу бота @getmyid_bot"
echo "   5. Скопируйте 'Chat ID' (он начинается на -100...)"
echo -n "👉 Введите ID группы (вместе с минусом): "
read CHAT_ID

# --- ШАГ 4: КОНТАКТ БАНА ---
echo -e "\n${YELLOW}4️⃣  ШАГ ЧЕТВЕРТЫЙ: Контакт для разбана${NC}"
echo "   Куда писать пользователю, если он словил бан?"
echo "   Пример: @admin_support"
echo -n "👉 Введите юзернейм или текст: "
read BAN_CONTACT
...
# Инициализация базы с настройкой контакта
echo "Инициализация базы данных..."
$INSTALL_DIR/venv/bin/python -c "import aiosqlite, asyncio; 
async def s():
    async with aiosqlite.connect('$INSTALL_DIR/bot.db') as db:
        await db.execute('CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)'); 
        await db.execute('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ('ban_contact', '$BAN_CONTACT')); 
        await db.commit()
asyncio.run(s())" 2>/dev/null
